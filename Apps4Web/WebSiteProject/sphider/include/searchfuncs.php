...
//		$a = remove_accents($a);
	...
	function search($searchstr, $category, $start, $per_page, $type, $domain) {
		...
		$possible_to_find = 1;
		$result = mysqli_query("select domain_id from ".$mysqli_table_prefix."domains where domain = '$domain'");
		...
		if ((count($result_array_full) == 0 || $possible_to_find == 0) && $did_you_mean_enabled == 1) {
			...
			foreach ($searchstr['+'] as $word) {
				...
				$result = mysqli_query("select keyword from ".$mysqli_table_prefix."keywords where soundex(keyword) = soundex('$word')");
				$max_distance = 100;
				$near_word ="";
				while ($row=mysqli_fetch_row($result)) {
		...
		while ($row = mysqli_fetch_row($result)) {
			...
			$dom_result = mysqli_query("select domain from ".$mysqli_table_prefix."domains where domain_id='".$domains[$row[0]]."'");
