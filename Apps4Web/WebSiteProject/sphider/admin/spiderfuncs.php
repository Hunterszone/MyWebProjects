...
function save_keywords($wordarray, $link_id, $domain) {
	...
	while ($thisword = each($wordarray)) {
		...
		if (strlen($word)<= 30) {
			...
			if ($keyword_id  == "") {
                mysqli_query("insert into ".$mysqli_table_prefix."keywords (keyword) values ('$word')");
				if (mysqli_errno() == 1062) { 
					$result = mysqli_query("select keyword_ID from ".$mysqli_table_prefix."keywords where keyword='$word'");
					echo mysqli_error();
...
function isDuplicateMD5($md5sum) {
	...
	$result = mysqli_query("select link_id from ".$mysqli_table_prefix."links where md5sum='$md5sum'");
	echo mysqli_error();
...
function check_for_removal($url) {
	global $mysqli_table_prefix;
	global $command_line;
	$result = mysqli_query("select link_id, visible from ".$mysqli_table_prefix."links"." where url='$url'");
	echo mysqli_error();
	if (mysqli_num_rows($result) > 0) {
		$row = mysqli_fetch_row($result);
		$link_id = $row[0];
		...
		if ($visible > 0) {
			$visible --;
			mysqli_query("update ".$mysqli_table_prefix."links set visible=$visible where link_id=$link_id");
			echo mysqli_error();
		} else {
